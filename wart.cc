...
bool validateHooks(const std::string &line) {
  return line.starts_with("x11hooks ") || line.starts_with("waylandhooks ") ||
...
bool rcValidate(const std::string &filepath,
  ...
  while (std::getline(file, line)) {
    ...
    if (!(iss >> key >> value)) {
      PRINT_ERROR("")
      std::cerr << "Error: malformed line in config file: " << line
                << std::endl;
  ...
  if (config.find("interval") == config.end() ||
      !validateInterval(config["interval"])) {
    std::cerr << "Error: 'interval' must be an integer > 0" << std::endl;
    return false;
  }

  if (config.find("clean") == config.end() ||
      !validateBoolean(config["clean"])) {
...
bool wartExists() {
  ...
  if (rcValidate(wartConfig, config)) {
    ...
    std::cout << "Clean: " << (config["clean"] == "1" ? "Enabled" : "Disabled")
              << std::endl;
...
bool fetchWallpaper(
  ...
  std::string url =
      "https://bing.biturl.top/?resolution=" + config.at("resolution") +
...
void showStatus() {
  ...
  if (rcValidate(wartConfig, config)) {
    ...
    std::cout << "Interval: " << config["interval"] << " seconds" << std::endl;
    std::cout << "Clean mode: "
              << (config["clean"] == "1" ? "Enabled" : "Disabled") << std::endl;
...
bool retryOperation(std::function<bool()> operation, int maxRetries = 3,
  ...
  for (int attempt = 1; attempt <= maxRetries; ++attempt) {
    ...
    if (attempt < maxRetries) {
      logMessage(LogLevel::WARNING, "Operation failed, retrying in " +
...
void wartLoop(const std::unordered_map<std::string, std::string> &config) {
  ...
  while (running) {
    ...
    std::cout << "Sleeping for " << config.at("interval") << " seconds..."
              << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    logMessage(LogLevel::ERROR,
               "Exception in main loop: " + std::string(e.what()));
    removeLockFile();
    return 1;
  }
